...
def run_training_loop(context: TrainingProgressContext, config: TrainingConfig):
    ...
    for stage_config in config.stages:
        ...
        decay_lr = optimizers.cosine_decay(
            stage_config.learn_rate_hi,
            stage_config.steps_main,
            stage_config.learn_rate_lo,
        )
        if stage_config.steps_warmup > 0:
            warmup_lr = optimizers.linear_schedule(
                stage_config.learn_rate_hi / 100.0,
                stage_config.learn_rate_hi,
                stage_config.steps_warmup,
            )
        ...
        with context.lock:
            ...
            context.iters_total = stage_config.steps_total()
        ...
        def get_batch_size(iter: int) -> int:
            if stage_config.batch_size > 0:
                return stage_config.batch_size
            assert len(stage_config.batch_size_list) > 0
            progress = iter / stage_config.steps_total()
            result = 1
            for threshold, count in stage_config.batch_size_list:
